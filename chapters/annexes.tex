...
\newpage
\section*{Annex H --- Clues from Playtests}
\addcontentsline{toc}{section}{Annex H --- Clues from Playtests}

The following catalogue enumerates behavioral observations, reported experiences, and emergent patterns from both playtest sessions that constitute evidence relevant to the five evaluation fields used in the Results chapter. Each entry states what was observed, what it points toward, the theoretical mechanism connecting it to the relevant outcome, and an assessment of evidential strength (Strong / Moderate / Weak).
...
\item \textbf{``Surprisingly liberating'' / ``a lot less anxiety''} (P1, D1)\\
\textit{Points toward:} Positive affective engagement; potential for voluntary repeated participation.\\
\textit{Why:} Emotional valence of first encounters strongly predicts willingness to re-engage. Absence of anxiety without absence of engagement is a difficult balance; its presence here suggests the transformational container is functioning as intended.\\
\textit{Strength:} Strong. Multiple independent reports across first playtest.

\item \textbf{Participants said they would play again and would ``try to be more emotionally immersed'' next time} (P1, D1)\\
\textit{Points toward:} Replayability; players perceive unexplored depth.\\
\textit{Why:} The desire to deepen engagement on replay indicates the format rewards mastery and exploration, a key condition for sustained community formation around a game.\\
\textit{Strength:} Moderate. Self-reported intention; actual re-engagement behaviour untested.

\item \textbf{Spontaneous adoption of ``inner voices'' terminology by players in second playtest} (P2, T2)\\
\textit{Points toward:} Imaginative investment; format's fictional premise genuinely inhabited.\\
\textit{Why:} Organic language generation signals that players were not merely following procedure but engaging imaginatively with the format's roleplay premise. This is a behavioral indicator rather than self-report, making it more credible.\\
\textit{Strength:} Strong. Unsolicited, behavioral, cross-confirmed by transcript.

\item \textbf{Women and less-successful-in-traditional-debate participants expressed strong preference for Inside Out} (P1, P2, D1, D2)\\
\textit{Points toward:} Broadened appeal; format reaching constituencies underserved by traditional debate.\\
\textit{Why:} Preference expressed independently by similar profiles across two separate sessions suggests a structural feature of the format rather than coincidence. Consistent with the design goal of redistributing who can succeed.\\
\textit{Strength:} Strong. Consistent across both playtests; theoretically coherent.

\item \textbf{One highly competitive debater found the format ``anticlimactic'' and missed ``total mind-domination''} (P1, D1)\\
\textit{Points toward:} Format is not a direct replacement for traditional debate but serves a different audience.\\
\textit{Why:} This negative reaction from a specific profile (dominant competitive performer) is itself informative: it confirms the format is restructuring what is rewarded, as designed. The response is coherent with the format's intent.\\
\textit{Strength:} Moderate. Single report; theoretically coherent and useful for scoping the target audience.
...
\item \textbf{``Architecting solutions very complex and interesting strategically''} (P1, D1)\\
\textit{Points toward:} Cognitive challenge maintained despite restructured format.\\
\textit{Why:} Players reporting the task as complex confirms the format does not simply reduce difficulty---it redirects it from rhetorical speed and coverage toward contextual reasoning and value argumentation.\\
\textit{Strength:} Moderate. Self-report of subjective difficulty; directionally strong.

\item \textbf{Subject player ran out of Huddle questions before 10 minutes elapsed} (P2)\\
\textit{Points toward:} Subject role is genuinely demanding; requires cognitive preparation and sustained presence.\\
\textit{Why:} Running out of facilitated questions is a design pressure point revealing that the Subject role needs scaffolding to sustain the Huddle productively. It simultaneously confirms that the role creates real intellectual demand.\\
\textit{Strength:} Moderate. Single observed instance; could reflect individual variation or a design gap requiring further testing.

\item \textbf{Commendations were challenging to assign without clear performance targets} (P2, D2)\\
\textit{Points toward:} Recognition system not yet calibrated for small, informal settings; needs an established circuit to become organic.\\
\textit{Why:} Difficulty assigning commendations is a weak negative clue suggesting the pressure/recognition mechanism is immature at this stage, but the evaluator's observation that memorized commendations become ``more organic'' points toward a viable long-term system.\\
\textit{Strength:} Weak (current scale); Moderate (projected at circuit scale).
...
\item \textbf{All Value Defenders in second playtest immediately adopted roleplay stance: second-person address, eye contact with Subject, worldbuilding details} (P2, T2)\\
\textit{Points toward:} Spontaneous perspective-taking; format reliably elicits character inhabitation.\\
\textit{Why:} Unsolicited roleplay behavior is a behavioral indicator of the imaginative engagement required for memetic and emancipatory bleed. Players spontaneously inhabiting their role suggests the format successfully creates the psychological conditions for transformation.\\
\textit{Strength:} Strong. Behavioral, cross-confirmed by transcript, unsolicited.

\item \textbf{Subject's opening speech: player emotionally contextualized scenario, exposing ``nuanced biases that bled from player to character''} (P2, T2)\\
\textit{Points toward:} Bleed from player into character; format activating genuine value reflection.\\
\textit{Why:} The Subject's first speech---describing the scenario in personal, emotional terms---created a vector for memetic bleed. Participants explicitly noted this as an institutionalization of existing debate practice (``playing for the judge'') in a more transparent and intentional form.\\
\textit{Strength:} Strong. Directly theoretically predicted; observed in transcript; verbally noted by participants.

\item \textbf{Participants articulated learning to ``think about context of decisions,'' to be ``more human and more honest about human nature''} (P1, D1)\\
\textit{Points toward:} Cognitive and attitudinal growth aligned with format goals.\\
\textit{Why:} Self-articulated growth in the precise domains the format targets (contextual reasoning, empathy, human complexity) is the most direct evidence available in the absence of pre/post measurement instruments.\\
\textit{Strength:} Moderate. Self-report; retrospective; no pre-measure available for comparison.

\item \textbf{Participants accepted that ``sometimes decisions are personal, not universal''} (P1, D1)\\
\textit{Points toward:} Epistemic shift toward contextual rather than universalist reasoning.\\
\textit{Why:} Traditional debate rewards universal claims. This reflection signals internalization of the format's procedural rhetoric: that value decisions are contextually embedded and legitimately vary by person and circumstance.\\
\textit{Strength:} Moderate. Single session; self-report; aligns with theoretical prediction.
...
\item \textbf{``Outright attacking others feels wrong, disrespectful, and unnecessary''---emergent norm in second playtest} (P2, D2)\\
\textit{Points toward:} Spontaneous formation of collaborative rhetorical norms.\\
\textit{Why:} This was not a stated rule but an emergent participant behavior: the format's procedural structure produced this norm without explicit instruction. This is a paradigmatic instance of procedural rhetoric reshaping community conduct.\\
\textit{Strength:} Strong. Behavioral; emerged spontaneously; verbally articulated by multiple participants.

\item \textbf{``Much much less anxiety-inducing'' than standard debate---consistent across both playtests} (P1, P2, D1, D2)\\
\textit{Points toward:} Transformational container functioning; psychological safety achieved.\\
\textit{Why:} Anxiety reduction is a structural prerequisite for the emancipatory and memetic bleed the format aims to support. Consistent reporting across independent groups strengthens confidence in this finding.\\
\textit{Strength:} Strong. Consistent across two independent sessions; multiple independent reports.

\item \textbf{Shifting focus and building on others' arguments felt ``more natural and more persuasive'' than rebuttal} (P2, D2)\\
\textit{Points toward:} Rhetorical culture shift; collaborative argumentation becoming the default competitive strategy.\\
\textit{Why:} When collaborative framing is perceived as more effective than adversarial framing, the competitive incentives of the format are aligned with its transformative goals. This is a coherent, self-reinforcing design outcome.\\
\textit{Strength:} Moderate. Single playtest; self-report; theoretically coherent.

\item \textbf{Format still provided ``structure for interesting conversation unlike open conversations that push toward fighting over talking space''} (P2, D2)\\
\textit{Points toward:} Format occupies a productive middle ground---structured without being combative.\\
\textit{Why:} This participant explicitly distinguishes Inside Out from both traditional debate (high anxiety, adversarial) and unstructured discussion (chaotic, dominated by loudest voices). This positions the format precisely where it was designed to sit.\\
\textit{Strength:} Moderate. Single well-articulated report; high theoretical resonance.
...
\item \textbf{``Every team says valuable stuff and it is important more to see what is important to us''} (P1, D1)\\
\textit{Points toward:} Epistemic humility: recognition that multiple competing perspectives hold genuine validity.\\
\textit{Why:} This is the core attitudinal outcome targeted by the design. Explicit self-report of this attitude after a first encounter---without prompting---is notable evidence of the format's potential to shift perspective.\\
\textit{Strength:} Moderate. Single playtest; self-report; directly on-target with design intent.

\item \textbf{Spontaneous consensus drift in second round of Value Defender speeches} (P2, T2)\\
\textit{Points toward:} Players orienting toward value integration rather than competitive dominance.\\
\textit{Why:} In traditional debate, second speeches intensify conflict. Here, players naturally moved toward synthesis without instruction. This procedural outcome is consistent with the format's design and constitutes a behavioral rather than self-reported indicator.\\
\textit{Strength:} Strong. Behavioral; transcript-confirmed; emergent; theoretically predicted.

\item \textbf{Unplanned bleed event: participant shared during a break that the debate scenario ``hit uncannily close to home''---a family member had asked them that day to provide false testimony} (P2)\\
\textit{Points toward:} Format successfully activating personal value reflection and real-world epistemic engagement.\\
\textit{Why:} Unplanned bleed---where a fictional scenario activates genuine personal stakes---is the strongest available indicator that the transformational container is functioning. The fact that this was shared spontaneously and privately (not as a performance for the researcher) increases its credibility substantially.\\
\textit{Strength:} Strong. Behavioral; spontaneous; private; paradigmatic instance of transformative bleed.

\item \textbf{Subject player noted conscious transition from roleplay mindframe (S0 speech) to evaluative mindframe (S2 speech) as debate progressed} (P2, T2)\\
\textit{Points toward:} Format produces genuine movement between epistemic stances within a single round.\\
\textit{Why:} The arc from emotionally inhabited Subject opening to reflective evaluative closing demonstrates the format's capacity to move participants through different modes of reasoning---experiential, empathic, then analytical---in a single structured encounter.\\
\textit{Strength:} Moderate. Single participant report; theoretically significant; coherent with format design.
